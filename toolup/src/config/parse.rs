...
pub fn initialize_configs(args: &ArgMatches) {
    ...
    let config_container = ConfigContainer { lock_config_path, lock_config: None };
...
fn read_existing_lock(global_path: &PathBuf, args: &ArgMatches) -> Result<ToolLock, CliError> {
    ...
    let lock_file = match fs::read_to_string(&global_path) {
        Ok(contents) => {
            match serde_json::from_str::<ToolLock>(&contents) {
                Ok(mut config) => {
                ...
                Err(_) => {
                    err!(ConfigError::ConfigFormatError(s!("Unable to deserialize existing state file.")))
                }
            }
        },
        Err(_) => err!(ConfigError::ConfigFormatError(s!("Unable to deserialize existing state file.")))
    };
