...
pub enum GraphQlRequest {
    GetReleases { owner: String, repo: String, artifact_name: String, limit: i32 }
}

#[derive(Serialize, Deserialize, Debug)]
#[serde(untagged)]
enum GraphQlVariables {
    GetReleases { owner: String, repo: String, artifact_name: String, release_count: i32}
}
...
fn parse_get_release_response(body: JsonValue) -> Result<Vec<VersionUrlResponse>, CliError> {
        ...
        match &object["data"] {
                ...
                if let JsonValue::Object(repo) = &data["repository"] {
                    if let JsonValue::Object(releases) = &repo["releases"] {
                        if let JsonValue::Array(nodes) = &releases["nodes"] {
                            return Ok(nodes.into_iter().map(|x| {
            ...
            _ => { 
                let error_message = if let JsonValue::Array(errors) = &object["errors"] {
                    let error_messages: Vec<String> = errors.into_iter().map(|x| {
    ...
    err!(ApiError::CallWasNotSuccessful(s!("Unable to parse GitHub error response")))
}
...
fn make_graph_ql_body(vars: GraphQlRequest) -> String {
    let query: GraphQlQuery = match vars {
        GraphQlRequest::GetReleases{ owner, repo, artifact_name, limit } => {
            GraphQlQuery { 
                query: GET_RELEASES.replace("\n", ""), 
                variables: GraphQlVariables::GetReleases { owner, repo, artifact_name, release_count: limit }
            }
        }
    };
